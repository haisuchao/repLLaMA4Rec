...
class DenseModel(EncoderModel):
    ...
    def _encode(self, x):
        hidden_states = self.encoder(**x, return_dict=True)
        hidden_states = hidden_states.last_hidden_state
        return self._pooling(hidden_states, x['attention_mask'])

    def encode_query(self, qry):
        # no-op unless untie_encoder=True, in which case this switches to the
        # query-only LoRA adapter before the forward pass (see EncoderModel._set_adapter)
        self._set_adapter('query')
        return self._encode(qry)

    def encode_passage(self, psg):
        # tied encoder (default): identical to encode_query, same adapter/weights
        # untied encoder: switches to the passage-only LoRA adapter instead
        self._set_adapter('passage')
        return self._encode(psg)
