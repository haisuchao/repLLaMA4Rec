...
7. [Data Augmentation — Sliding Window](#7-data-augmentation--sliding-window-export_tevatron_augpy)
8. [So sánh hai định dạng](#8-so-sánh-hai-định-dạng)
...
│                            #   - Leave-one-out split
│
├── export_tevatron.py       # Xuất định dạng Tevatron v2 — 1 sample/user
├── export_tevatron_aug.py   # Xuất định dạng Tevatron v2 — sliding window augmentation
├── export_recbole.py        # Xuất định dạng RecBole (SASRec)
├── run_all.py               # Entry point — chạy toàn bộ pipeline (export_tevatron + export_recbole)
│
...
Preprocessing result
      ├──→ export_tevatron.py      →  dataset/tevatron/<dataset>/       (1 sample/user)
      ├──→ export_tevatron_aug.py  →  dataset/tevatron/<dataset>-aug/   (N-3 samples/user)
...
## 7. Data Augmentation — Sliding Window (`export_tevatron_aug.py`)
...
# window_size mặc định = CONTEXT_SIZE = 3, output vào beauty-aug/
python export_tevatron_aug.py beauty

# Thử window_size khác, output vào beauty-aug-w5/
python export_tevatron_aug.py beauty --window_size 5 --tag aug-w5

# ML-1M: nên giới hạn số samples/user để tránh train quá lâu (avg=165 items)
...
|---|---|---|
| `--window_size` | `CONTEXT_SIZE=3` | Số item tối đa trong context query |
| `--max_aug_per_user` | `None` (unlimited) | Giới hạn số samples/user, lấy K vị trí **gần nhất** |
| `--tag` | `aug` hoặc `aug-w{N}` | Hậu tố thư mục output |
...
Các tham số chỉnh trong `export_tevatron.py` và `export_tevatron_aug.py`:

| Tham số | Mặc định | Ý nghĩa |
|---|---|---|
| `NUM_NEGATIVES` | `50` | Số negative sample mỗi query (train only) |

Tham số CLI riêng của `export_tevatron_aug.py`:

| Tham số | Mặc định | Ý nghĩa |
|---|---|---|
| `--window_size` | `CONTEXT_SIZE=3` | Kích thước cửa sổ context |
| `--max_aug_per_user` | `None` | Giới hạn samples/user (None = unlimited) |
| `--tag` | `aug` / `aug-w{N}` | Hậu tố thư mục output |
