...
7. [Query Augmentation — Sliding Window](#7-query-augmentation--sliding-window)
8. [BM25 Hard Negative Mining](#8-bm25-hard-negative-mining)
9. [So sánh hai định dạng](#9-so-sánh-hai-định-dạng)
...
Preprocessing result
      ...
      │           (mặc định)          → beauty/         1 sample/user, random negatives
      │           --window_size 3     → beauty-w3/      aug, random negatives
      │           --neg_strategy mixed → beauty-mixed/  1 sample/user, hard+random negatives
      │           --window_size 3
      │           --neg_strategy mixed → beauty-w3-mixed/  aug + hard+random negatives
...
# window=3, output → beauty-w3/
python export_tevatron.py beauty --window_size 3

# window=5, output → beauty-w5/
python export_tevatron.py beauty --window_size 5

# ML-1M: giới hạn 20 samples/user để tránh train quá lâu (avg=165 items), output → ml-1m-w3/
python export_tevatron.py ml-1m --window_size 3 --max_aug_per_user 20
```

Train sau khi export (tag tự động `w3`):

```bash
./train.sh beauty Qwen/Qwen3-Embedding-0.6B 8 w3
./eval.sh beauty best Qwen/Qwen3-Embedding-0.6B test w3
```
...
|---|---|---|
| `--window_size N` | `None` (tắt) | Kích thước cửa sổ context, bật augmentation |
| `--max_aug_per_user N` | `None` (unlimited) | Giới hạn samples/user, lấy K vị trí **gần nhất** |
| `--tag TAG` | tự động (`w{N}`) | Override hậu tố thư mục output |

### Ước tính số samples

| Dataset | Users | Avg seq len | Samples (standard) | Samples (aug, unlimited) |
|---|---|---|---|---|
| Beauty | 22,363 | 8.9 | ~22k | ~154k (~7×) |
| Sports | 35,598 | 8.3 | ~36k | ~224k (~6×) |
| ML-1M | 6,040 | 165.5 | ~6k | ~980k (~163×) — dùng `--max_aug_per_user 20` |

---

## 8. BM25 Hard Negative Mining
...
# Kết hợp augmentation + mixed negatives, output → beauty-w3-mixed/
python export_tevatron.py beauty --window_size 3 --neg_strategy mixed

# ML-1M: augmentation + mixed, output → ml-1m-w3-mixed/
...
```bash
./train.sh beauty Qwen/Qwen3-Embedding-0.6B 8 mixed
./eval.sh beauty best Qwen/Qwen3-Embedding-0.6B test mixed
```
...
| `--tag TAG` | tự động (`mixed`, `bm25`, `w3-mixed`, ...) | Override hậu tố thư mục output |
...
| Tham số | Mặc định | Ý nghĩa |
|---|---|---|
| `--window_size N` | `None` (tắt) | Bật augmentation với sliding window kích thước N |
| `--max_aug_per_user N` | `None` (unlimited) | Giới hạn samples/user khi augmentation |
| `--neg_strategy` | `random` | `random` / `bm25` / `mixed` |
| `--num_hard_neg N` | `10` | Số BM25 hard negatives mỗi query |
| `--num_random_neg N` | `40` | Số random negatives mỗi query |
